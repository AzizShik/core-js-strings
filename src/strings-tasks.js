...
function sumOfCodes(str) {
  return str
    ? str.split('').reduce((acc, letter) => acc + letter.charCodeAt(), 0)
    : 0;
}
...
function countVowels(str) {
  return Array.from(str).filter((letter) => 'aeiouyAEIUOY'.includes(letter))
    .length;
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((letter) =>
      rot13Alphabet[normalAlphabet.indexOf(letter)]
        ? rot13Alphabet[normalAlphabet.indexOf(letter)]
        : letter
    )
    .join('');
